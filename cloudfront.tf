...
resource "aws_cloudfront_distribution" "content_distribution" {
  web_acl_id = aws_wafv2_web_acl.honeypot_waf_acl.arn
  ...
  ordered_cache_behavior {
    ...
    lambda_function_association {
      ...
      include_body = false
      lambda_arn   = "${aws_lambda_function.waf_lambda.arn}:${aws_lambda_function.waf_lambda.version}"
    }
  }
